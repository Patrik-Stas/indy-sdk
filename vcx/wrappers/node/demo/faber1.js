...
async function runFaber (options) {
  ...
  fs.writeFileSync('faber1_provision.json', JSON.stringify(agentProvision))
  ...
  logger.warn(`Sent offer to alice. Alice connection = ${JSON.stringify(connectionToAlice)}`)

  const serCred = await credentialForAlice.serialize()
  const serCredString = JSON.stringify(serCred, null, 2)
  logger.info(`Storing credential: ${serCredString}`)
  fs.writeFileSync('credentialForAlice.json', serCredString)

  const serConn = await connectionToAlice.serialize()
  const serConnString = JSON.stringify(serConn, null, 2)
  logger.info(`Storing connection: ${serConnString}`)
  fs.writeFileSync('connectionToAlice.json', serConnString)
}
